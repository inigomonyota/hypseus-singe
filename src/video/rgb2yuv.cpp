...
// Modern, high-performance C++ implementation
...
void rgb2yuv() {
    ...
    // Clamp Y to 0-255 range
    if (y_val < 0) y_val = 0;
    else if (y_val > 255) y_val = 255;
    ...
    // Clamp U to 0-255 range
    if (u_val < 0) u_val = 0;
    else if (u_val > 255) u_val = 255;
    ...
    // Clamp V to 0-255 range
    if (v_val < 0) v_val = 0;
    else if (v_val > 255) v_val = 255;
