...
void splash() {
    ...
    SDL_RWops* ops = SDL_RWFromConstMem(ghc, (int)sizeof(ghc));
    ...
    logical.w = fs ? (int)video::get_logical_width() : (int)video::get_viewport_width();
    logical.h = fs ? (int)video::get_logical_height() : (int)video::get_viewport_height();

    if (!fs) scale = FC_MakeScale(SCALE, SCALE);

    const char* v = get_hypseus_version();
    const int f = fs ? 2 : 1;

    // C++-legal SDL_Rect initialization (x,y,w,h)
    SDL_Rect dscrect{
        (int)(((logical.w - DSC) >> 1) + (EMBW >> 1) + 0xE),
        (int)(((logical.h - EMBH) >> 1) + (f >> 2)),
        DSC,
        DSC
    };

    SDL_Rect embrect{
        (int)(((logical.w - EMBW) >> 1) - (f << 2)),
    ...
    for (int i = 0; i < FRAMES; ++i)
    {
        ...
        SDL_RenderCopy(renderer, splash_frames, &frames[frames_indices[current_frame]], &dscrect);

        if (i > 0xA)
            FC_DrawEffect(font, renderer, (logical.w >> 1), (logical.h * 90) / 100, effect, v);
