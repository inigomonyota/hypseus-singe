...
#define LOCK_AUDIO()                                                           \
    do { assert(!g_bAudioLocked);                                              \
    g_bAudioLocked = true;                                                     \
    SDL_LockAudioDevice(g_audio_device); } while(0)
#define UNLOCK_AUDIO()                                                         \
    do { assert(g_bAudioLocked);                                               \
    g_bAudioLocked = false;                                                    \
    SDL_UnlockAudioDevice(g_audio_device); } while(0)
#else
#define LOCK_AUDIO() SDL_LockAudioDevice(g_audio_device)
#define UNLOCK_AUDIO() SDL_UnlockAudioDevice(g_audio_device)
#endif
    ...
    bool init() {
        ...
        if (is_enabled()) {
            if (SDL_InitSubSystem(SDL_INIT_AUDIO) >= 0) {
                ...
                // Use SDL_OpenAudioDevice with allowed_changes = 0
                // This tells SDL to convert internally if the hardware
                // doesn't natively support our requested format.
                ...
                g_audio_device = SDL_OpenAudioDevice(
                    ...
                    &specDesired,
                    &specObtained,
                    0               // no allowed changes — SDL converts for us
                );
